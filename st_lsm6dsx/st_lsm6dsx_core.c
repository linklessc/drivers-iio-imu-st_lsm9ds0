...
static int st_lsm6dsx_init_device(struct st_lsm6dsx_hw *hw)
{
    ...
    /* --- Modified: Force enable Accel LPF2 (Low Pass Filter) --- */
    // Set Bit 1 (LPF2_XL_EN) of CTRL1_XL (0x10)
    // This enables hardware-level filtering of high-frequency noise.
    err = regmap_update_bits(hw->regmap, 0x10, BIT(1), BIT(1));
    if (err < 0)
        dev_warn(hw->dev, "Failed to enable LPF2\n");
    /* --------------------------------------------------- */

    // Keep the original timer initialization as the end of the function
					       ...
					       const char *name)
{
	...
	sensor->id = id;
	sensor->hw = hw;
	//sensor->odr = hw->settings->odr_table[id].odr_avl[5].milli_hz;  // odr_avl[5] usually corresponds to 416 Hz
/* --- Start of modification: Support reading ODR from DTS --- */
    u32 dts_odr_hz = 0;
    int err;

    // 1. Set a default value first (e.g., 26Hz) in case the DTS entry is missing
    sensor->odr = hw->settings->odr_table[id].odr_avl[1].milli_hz; 

    // 2. Attempt to read the DTS property "st,init-odr"
    err = device_property_read_u32(hw->dev, "st,init-odr", &dts_odr_hz);
    if (!err) {
        // If read is successful, convert Hz to mHz (since the driver uses milli-hertz internally)
        u32 target_mhz = dts_odr_hz * 1000;
        int i;
        const struct st_lsm6dsx_odr_table_entry *odr_table = &hw->settings->odr_table[id];
        bool found = false;

        // 3. Search for a matching value in the list of supported frequencies
        for (i = 0; i < odr_table->odr_len; i++) {
            if (odr_table->odr_avl[i].milli_hz == target_mhz) {
                sensor->odr = target_mhz;
                found = true;
                dev_info(hw->dev, "Successfully set initial ODR from DTS: %u Hz\n", dts_odr_hz);
                break;
            }
        }
        
        if (!found) {
            dev_warn(hw->dev, "ODR %u Hz defined in DTS is not supported by this sensor, using default.\n", dts_odr_hz);
        }
    }
/* --- End of modification --- */
